...
const Background = ({ children }) => {
  return (
    <View>
      <ImageBackground source={require("../assets/scan_renter.jpg")} style={{ height: '80%'}} />
...
const ScanRenterId = ({ navigation, route }) => {
  const { renterIdData, setRenterIdData } = useContext(ContractContext);
  ...
  const [domiciliuChirias, setDomiciliuChirias] = useState('');
  const [nrBuletinChirias, setNrBuletinChirias] = useState('');
  const [numeChirias, setNumeChirias] = useState('');
  const [prenumeChirias, setPrenumeChirias] = useState('');
  const [serieBuletinChirias, setSerieBuletinChirias] = useState('');
  ...
  const scannIdHandler = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setScannedData(data);
        setDomiciliuChirias(data.domiciliu);
        setNrBuletinChirias(data.nrBuletin);
        setNumeChirias(data.nume);
        setPrenumeChirias(data.prenume);
        setSerieBuletinChirias(data.serieBuletin);
            ...
            {scannedData && (
              <View style={stylesChoosePhoto.overlay}>
                <TextInput
                  ...
                  value={serieBuletinChirias}
                  onChangeText={text => setSerieBuletinChirias(text)}
                />
                <TextInput
                  style={stylesChoosePhoto.nrBuletin}
                  value={nrBuletinChirias}
                  onChangeText={text => setNrBuletinChirias(text)}
                />
                <TextInput
                  style={stylesChoosePhoto.numeBuletin}
                  value={numeChirias}
                  onChangeText={text => setNumeChirias(text)}
                />
                <TextInput
                  style={stylesChoosePhoto.prenumeBuletin}
                  value={prenumeChirias}
                  onChangeText={text => setPrenumeChirias(text)}
                />
                <TextInput
                  style={stylesChoosePhoto.adresaBuletin}
                  value={domiciliuChirias}
                  onChangeText={text => setDomiciliuChirias(text)}
                />
              </View>
            )}
          </View>
