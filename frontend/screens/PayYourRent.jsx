...
const PayYourRent = ({ navigation, route}) => {
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        // Define a function to fetch the sumaChirie from the server
        const fetchSumaChirie = async () => {
          try {
            // Make the HTTP GET request to your backend API
            const response = await axios.get('http://127.0.0.1:5000/api/v1/getsumachirie');
    
            // Extract the sumaChirie from the response data
            const { sumaChirie } = response.data;
            console.log(response)
            console.log(sumaChirie)
            // Set the sumaChirie state
            setRentAmount(sumaChirie);
            console.log("AICI",rentAmount)
          } catch (error) {
            console.error(error);
          }
        };
        // Call the fetchSumaChirie function
        fetchSumaChirie();
      }, []);
      ...
      const handlePassword = async () => {
        try {
          ...
          if (response.ok) {
            navigation.navigate("striperentgateway", {selectedCard, rentAmount})
          } else {
    ...
    return (
        <ScrollView style={styles.containerBig}>
            <Text style={{ fontSize: 20, marginLeft: 20,marginTop: 20,fontWeight: 'bold' }}>Pay Your Rent</Text>
